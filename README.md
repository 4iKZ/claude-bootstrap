...
| `NODE_DIST_MIRROR_FALLBACK`                | npmmirror Node.js 镜像              | 官方 Node.js 源不可用时的回退地址   |
| `NVM_NODEJS_ORG_MIRROR`                    | —                                  | Linux/macOS 自定义 Node.js 下载源   |
| `FNM_NODE_DIST_MIRROR`                     | —                                  | Windows 自定义 Node.js 下载源       |
...
安装 Node.js 22（通过 nvm）
  ├─ nvm 使用官方 script 模式，不执行 Git Clone
  └─ Node.js 官方源不可用时回退 npmmirror
        │
        ▼
...
<details>
<summary><strong>nvm 安装时无法连接 github.com</strong></summary>

脚本使用 nvm 官方 `METHOD=script` 模式，通过 `raw.githubusercontent.com` 下载 nvm 文件，不再执行 `git clone github.com/nvm-sh/nvm.git`。安装 Node.js 时会先检测官方源，连接失败则自动回退 npmmirror；也可以通过 `NVM_NODEJS_ORG_MIRROR` 或 `FNM_NODE_DIST_MIRROR` 显式指定下载源。
...
直接重新运行脚本。检测到已有配置后，可选择保留现有配置、仅切换模型或重新配置全部：
