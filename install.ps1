...
function Fetch-ModelIdsFromGateway {
    ...
    try {
        $response = Invoke-WebRequest -Uri $url -Headers @{ "Authorization" = "Bearer $Secret" } -TimeoutSec 15 -UseBasicParsing -ErrorAction Stop
        if ($AuthMode -eq "api_key") {
            $response = Invoke-WebRequest -Uri $url -Headers @{ "X-Api-Key" = $Secret } -TimeoutSec 15 -UseBasicParsing -ErrorAction Stop
        }
...
function Write-EnvFile {
    ...
    if ($AuthMode -eq "auth_token") {
        $lines += "`$env:ANTHROPIC_AUTH_TOKEN = '$Secret'"
        $lines += "Remove-Item env:ANTHROPIC_API_KEY -ErrorAction SilentlyContinue"
    } else {
        $lines += "`$env:ANTHROPIC_API_KEY = '$Secret'"
        $lines += "Remove-Item env:ANTHROPIC_AUTH_TOKEN -ErrorAction SilentlyContinue"
    }
    $lines += "`$env:ANTHROPIC_MODEL = '$Model'"
    $lines += "`$env:ANTHROPIC_CUSTOM_MODEL_OPTION = '$Model'"
    $lines += "`$env:CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY = '$ENABLE_GATEWAY_MODEL_DISCOVERY'"
    $lines += "`$env:CLAUDE_CODE_SUBPROCESS_ENV_SCRUB = '$CLAUDE_CODE_SUBPROCESS_ENV_SCRUB_DEFAULT'"
    ...
    if (Test-Path $ENV_FILE) {
        ...
        $psEnv -split "`r?`n" | ForEach-Object {
            if ($_ -match "^\`$env:(\w+)\s*=\s*'([^']*)'") {
                $cmdLines += "set $($Matches[1])=$($Matches[2])"
            }
        }
